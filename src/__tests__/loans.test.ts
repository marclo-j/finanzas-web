...
describe("Loans CRUD", () => {
  ...
  it("CREATE - inserta una deuda", async () => {
    const [row] = await db!.insert(loans).values({
      person: "Juan Pérez", desc: "Préstamo para mudanza", amount: 500,
  ...
  it("READ - lista deudas", async () => {
    await db!.insert(loans).values([
      { person: "Ana", desc: "Deuda 1", amount: 200, lentDate: "2026-06-01", dueDate: "2026-08-01", status: "pendiente", paidAmount: 0 },
      { person: "Luis", desc: "Deuda 2", amount: 150, lentDate: "2026-05-15", dueDate: "2026-07-15", status: "pendiente", paidAmount: 0 },
    ]);
  ...
  it("UPDATE - marca deuda como pagada", async () => {
    const [created] = await db!.insert(loans).values({
      person: "María", desc: "Test", amount: 300, lentDate: "2026-06-01", dueDate: "2026-08-01", status: "pendiente", paidAmount: 0,
    }).returning();

    const [updated] = await db!.update(loans)
      .set({ status: "pagada", paidAmount: 300 })
  ...
  it("DELETE - elimina una deuda", async () => {
    const [created] = await db!.insert(loans).values({
      person: "Pedro", desc: "Test delete", amount: 100, lentDate: "2026-06-01", dueDate: "2026-07-01", status: "pendiente", paidAmount: 0,
    }).returning();
